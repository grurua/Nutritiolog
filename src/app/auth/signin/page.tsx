...
function SignInContent() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (result?.error) {
        setFormError('არასწორი ელ-ფოსტა ან პაროლი');
      } else {
        window.location.href = callbackUrl;
      }
    } catch {
      setFormError('შეცდომა მოხდა. სცადეთ თავიდან.');
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="max-w-sm mx-auto px-4 py-12">
      <div className="bg-white rounded-2xl border border-gray-200 p-8 shadow-sm">
        <div className="text-center mb-6">
          <h1 className="text-2xl font-bold text-gray-900 mb-2">შესვლა</h1>
          <p className="text-gray-500 text-sm">გაიარე ავტორიზაცია კალკულატორის გამოსაყენებლად</p>
        </div>

        {(error || formError) && (
          <div className="bg-red-50 text-red-700 text-sm rounded-lg p-3 mb-4">
            {formError || 'ავტორიზაცია ვერ მოხერხდა. გთხოვთ, სცადოთ თავიდან.'}
          </div>
        )}
        ...
        {googleAvailable && (
          <>
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-white border border-gray-300 rounded-xl text-gray-700 font-medium hover:bg-gray-50 transition-colors"
            >
...
export default function SignInPage() {
  return (
    <Suspense
      fallback={
        <div className="max-w-sm mx-auto px-4 py-16 text-center text-gray-400">იტვირთება...</div>
      }
    >
      <SignInContent />
    </Suspense>
